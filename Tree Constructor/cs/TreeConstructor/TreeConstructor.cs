...
public static class TreeConstructor
{
    ...
    {
        var parentChildCount = new Dictionary<int, int>();
        var childParentCount = new Dictionary<int, int>();

        foreach (var pair in strArr)
        {
            var nums = pair.Trim('(', ')').Split(',');
            int child = int.Parse(nums[0]);
            int parent = int.Parse(nums[1]);

            parentChildCount[parent] = parentChildCount.GetValueOrDefault(parent) + 1;
            childParentCount[child] = childParentCount.GetValueOrDefault(child) + 1;

            if (parentChildCount[parent] > 2 || childParentCount[child] > 1)
            {
                return "false";
            }
        }
